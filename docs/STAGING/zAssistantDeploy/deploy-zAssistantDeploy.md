...
In this step you will deploy ***zAssistantDeploy*** to your OpenShift cluster. **zAssistantDeploy** provides a range of pre-configured services, including OpenSearch, authorization, z/OS topology, and data ingestion. These integrated components work together to facilitate comprehensive document management, covering indexing, ingestion, authentication, and retrieval. By configuring conversational search parameters in watsonx Assistant for Z, you can deliver more accurate and contextually relevant responses based on the available content sources.
...
2. In the `deploy-zad.yaml` file in the same directory, open the file and modify it in a text editor of your choice, substituting the domain name recorded in the previous step for the `<YOUR_CLUSTER_DOMAIN>` string.
