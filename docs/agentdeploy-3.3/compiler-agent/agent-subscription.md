...
Finally, subscribe the agent to automate the bootstrapping of the **Compiler Fix Finders Agent** to your watsonx Orchestrate environment. 



1. Copy and paste the following into a file called `compiler-subscribe.yaml`:

    ```
    apiVersion: wxa4z.watsonx.ibm.com/v1alpha1
    kind: AgentSubscription
    metadata:
      name: compiler-agent-z-subscription
      namespace: wxa4z-agents
    spec:
      agent:
        agent_id: wxa4z:compiler-fix-finder-agent:agent
...
4. After setting that value, subscribe your agent by running the following command:
    ...
    ```
    oc apply -f compiler-subscribe.yaml
    ```
